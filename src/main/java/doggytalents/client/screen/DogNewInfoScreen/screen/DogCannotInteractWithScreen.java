...
public class DogCannotInteractWithScreen extends Screen {
    ...
    protected void init() {
        ...
        showIncapStrButton = new TextOnlyButton(0, 0, font.width(showCause) + 5, font.lineHeight + 3,
            ...
            protected void extractContents(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float pTicks) {
                ...
                graphics.setComponentTooltipForNextFrame(font, msgList, mouseX, mouseY);
            }
        };
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float pTicks) {
        ...
        if (this.dog.isDefeated()) {
            ...
            title = Component.translatable("doggui.invalid_dog.no_permission.title", dog.getName().getString())
            .withStyle(
                Style.EMPTY
                .withBold(true)
    ...
    private void renderIncapScreen(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float pTicks)  {
        var stack = graphics.pose();
        int pX = this.width/2;
        int mY = this.height/2;
        ...
        var lines2 = font.split(Component.literal(help), 150);
        ...
        for (var line : lines1) {
            stack.pushMatrix();
            stack.scale(1.2f);
            graphics.text(font, line, Mth.floor(pX/1.2f), Mth.floor(pY/1.2f), 0xffffffff);
            stack.popMatrix();
            pY += 14;
        }
        pY += 7;
        ...
        graphics.text(font, escToReturn, escTX, escTY, 0xffffffff );
