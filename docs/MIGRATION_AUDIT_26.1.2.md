...
MiiRaGe's `26.1.2` branch was the correct implementation base, but it was not at feature parity or release-ready. The dominicbytes continuation now compiles, packages, passes 33 unit tests and seven DTN GameTests, and reaches a dedicated-server `Done` state against the current stable NeoForge 26.1.2 loader. Client/runtime evidence is still not broad enough to support the original branch's "GUI / client rendering done" claim.
...
- `test` passes 33 tests, and the NeoForge GameTest server runs seven registered DTN tests successfully. Java 25 CI passes.
...
- Forced Java compilation produces zero removal warnings against `.101`. The deprecated dog-bed tag-refresh and item-handler surfaces have both been migrated and regression-tested.

Evidence boundary: `COMPILES`, static packaging, 33 unit tests, seven DTN in-process GameTests, a representative frozen 1.21.1 dog-payload upgrade, development-classpath server startup, clean production-JAR dedicated-server startup, and production-JAR persistence across a stopped-process restart are proven. Client, full-world fixture upgrades, and visual parity are not yet proven. Multiplayer acceptance is explicitly waived for the dominicbytes fork and remains upstream work.

## Confirmed parity gaps

### P0 — release and multiplayer safety

1. **The automated harness is still incomplete.** JUnit covers network and storage compatibility, while NeoForge GameTests prove real-dog core serialization, a frozen 1.21.1 dog-payload upgrade, representative talents, dog-owned inventories, accessory color, artifact state, complete statistics, established food-bowl/dog-bed/rice-mill state, and nonzero rice-mill progress. A manual two-JVM production-server gate now proves a clean save, stop, restart, and reload of broad dog state. Automating that lifecycle and adding a captured full-world upgrade remain open; Java 25 CI remains required.
...
5. **Persistence and upgrade behavior is substantially proven.** Original 1.21.1 respawn-owner UUIDs, the interim 26.1.2 string UUID format, legacy location-list/entity-ID keys, canonical respawn re-encoding, and malformed-entry rejection have unit coverage. Loader-aware GameTests prove that a real `Dog` round-trips UUID, owner/tame/name, gender, mode, hunger, incapacitation value, normal/kami levels, behavior flags, bed/bowl locations, representative plain/stateful talents, Pack Puppy and Doggy Tools inventories/options, accessory color, an artifact, and complete statistics through the 26.1 `ValueInput`/`ValueOutput` entity bridge. A checksum-locked 1.21.1 payload additionally proves legacy dire-level and custom-skin fallback plus canonical rewrite. Production block-entity serialization preserves food-bowl inventory/placer, dog-bed materials/owner/names, rice-mill inventory, and canonical nonzero rice-mill progress. The production JAR also preserves core, loadout, location, statistics, group, restriction, and policy state across a clean two-JVM stop/restart; see `docs/SAVE_RESTART_QA_26.1.2.md`. The targeted respawn, armor, and location-migration failure paths now emit stable contextual diagnostics while preserving graceful fallback; see `docs/PERSISTENCE_DIAGNOSTICS_QA_26.1.2.md`. No authentic captured full-world fixture is available, so a provenance-verifiable full-world upgrade remains an optional evidence improvement.
...
15. **Deprecated NeoForge item-handler surface is complete.** Mod-owned inventories now use `ItemStacksResourceHandler`, menus use `ResourceHandlerSlot`, the rice mill uses `VanillaContainerWrapper`, and transfer helpers use atomic transactions. The established stack API and save schemas remain compatible. Forced compilation has zero item-handler removal warnings; see `docs/ITEM_HANDLER_MIGRATION_QA_26.1.2.md`.
...
| SAVE-01 | 1.21.1 fixture upgrades with dog identity, talents, accessories, inventories, locations, and respawn state intact | PASS for the frozen entity fixture and production stop/restart contract — unit compatibility tests, six in-process persistence GameTests, and the two-JVM `SAVE-01-RESTART` gate pass; a captured full-world upgrade remains a separate evidence improvement |
...
| ITEM-HANDLER-01 | Inventory transactions preserve validation, merge ordering, rollback, menu access, and established save schemas | PASS — loader-aware GameTest and `docs/ITEM_HANDLER_MIGRATION_QA_26.1.2.md` |
| GAMEPLAY-WHISTLE-01 | Valid legacy whistle modes remain stable; absent, negative, and oversized custom-data values safely default to stand mode | PASS — unit and loader-aware GameTests plus `docs/WHISTLE_MODE_COMPATIBILITY_QA_26.1.2.md` |
...
The item-handler branch replaces the 65 remaining removal warnings with NeoForge 26.1.2 transactional resource handlers. A small public adapter retains DTN's stack-oriented addon and dog-equipment contract while storage mutations use transactions. Menus, rice-mill output, generic insertion, and cross-inventory transfer use the target API directly. `ITEM-HANDLER-01` adds loader coverage for commit/rollback, validation, extraction limits, Treat Bag flushing, merge ordering, and failure atomicity; existing serialization tests now also cover dog armor. See `docs/ITEM_HANDLER_MIGRATION_QA_26.1.2.md`.

The first gameplay-sweep slice centralizes whistle mode decoding. All 23 stable legacy indexes remain unchanged, while missing, negative, and oversized `CUSTOM_DATA` values now resolve to `STAND` across dog interaction, normal use, duty-whistle detection, and stack naming. `GAMEPLAY-WHISTLE-01` passes focused unit coverage and a loader-aware test using the registered item; see `docs/WHISTLE_MODE_COMPATIBILITY_QA_26.1.2.md`.
...
The production-server gate installs the official checksum-verified NeoForge `26.1.2.101` server into a clean disposable directory, copies only the freshly built production JAR into `mods/`, and reaches `Done`. The strict Modmaker artifact validator also passes with zero findings. This closes dedicated-server packaging/classloading acceptance. The same installation and exact JAR subsequently pass the separate stopped-process save/restart gate; client acceptance remains open.
