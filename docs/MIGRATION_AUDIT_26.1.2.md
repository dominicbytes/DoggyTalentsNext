...
MiiRaGe's `26.1.2` branch was the correct implementation base, but it was not at feature parity or release-ready. The dominicbytes continuation now compiles, packages, passes 30 unit tests and five DTN GameTests, and reaches a dedicated-server `Done` state against the current stable NeoForge 26.1.2 loader. Client/runtime evidence is still not broad enough to support the original branch's "GUI / client rendering done" claim.
...
- `test` passes 30 tests, and the NeoForge GameTest server runs five registered DTN persistence tests successfully. Java 25 CI remains open.
...
- The exact packaged JAR (`da39b1f838e4fe0b9b01909778317a94f218a0ac62b24122c0e118e6d3b98cef`) also loads from a clean NeoForge `26.1.2.101` server `mods/` directory and reaches `Done (0.271s)` without a DTN classloading failure. See `docs/PRODUCTION_SERVER_QA_26.1.2.md`.
...
Evidence boundary: `COMPILES`, static packaging, 30 unit tests, five DTN in-process persistence GameTests, development-classpath server startup, and clean production-JAR dedicated-server startup are proven. Client, process-restart persistence, broad frozen-fixture upgrades, and visual parity are not yet proven. Multiplayer acceptance is explicitly waived for the dominicbytes fork and remains upstream work.

## Confirmed parity gaps

### P0 — release and multiplayer safety

1. **The automated harness is still incomplete.** JUnit covers network and storage compatibility, while NeoForge GameTests prove real-dog core serialization, representative talents, dog-owned inventories, accessory color, artifact state, established food-bowl/dog-bed/rice-mill state, and nonzero rice-mill progress. Add lifecycle, broad frozen-fixture, tracker, and restart coverage. Java 25 CI remains required.
...
| SAVE-01 | 1.21.1 fixture upgrades with dog identity, talents, accessories, inventories, locations, and respawn state intact | PARTIAL — unit compatibility tests plus five in-process persistence GameTests, including statistics, established block-entity state, and rice-mill progress; broad frozen fixtures and restart remain |
...
The dog persistence branches add the NeoForge `gameTestServer` run and five stable `SAVE-01` subgates. The harness registers explicit 26.1 `GameTestInstance`s, correcting the earlier function-only registration that did not create runnable tests. The core test asserts vanilla identity, core DTN state, and location maps. The extended test asserts talent levels, Pack Puppy and Doggy Tools inventory/options, dyed accessory state, and an artifact. The block-entity tests reconstruct production food-bowl, dog-bed, and rice-mill block entities and assert their established identity/material/name/inventory contracts plus canonical nonzero rice-mill progress. The statistics test covers every persisted counter and entity-kill entry, catches stale derived totals, and proves replacement-load semantics. All five DTN tests pass in the loader-aware server; broad frozen fixtures and process restart remain open.
